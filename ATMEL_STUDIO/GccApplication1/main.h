#ifndef MAIN_H_
#define MAIN_H_

// ----------------------------------------------------------------------------
#define F_CPU 1200000UL // SYSCLK=1.2MHz, HSI=9.6MHz, CKDIV8=1, Attiny13A

// ----------------------------------------------------------------------------
#include <avr/io.h>
#include <avr/portpins.h>
#include <avr/interrupt.h>
#include <util/delay.h>
#include <avr/pgmspace.h>

// ----------------------------------------------------------------------------
#define TRUE 1
#define FALSE 0
#define bool uint8_t
#define SET_BIT(REG, BIT) ((REG) |= (1<<BIT))
#define CLEAR_BIT(REG, BIT) ((REG) &= ~(1<<BIT))
#define READ_BIT(REG, BIT) ((REG) & (1<<BIT))
#define TOGGLE_BIT(REG, BIT) ((REG) ^= (1<<BIT))
#define MODIFY_REG(REG, CLEARMASK, SETMASK) (WRITE_REG((REG), (((READ_REG(REG)) & (~(CLEARMASK))) | (SETMASK))))

#define P0 1<<0
#define P1 1<<1
#define P2 1<<2
#define P3 1<<3
#define P4 1<<4
#define P5 1<<5
#define P6 1<<6
#define P7 1<<7

const int step = 2; // шаг изменени€ €ркости
const int max = 146; // максимальное значение €ркости (номер последнего элемента в массиве €ркостей минус 1)

int brightness;     // уровень €ркости от 0 до 255

uint8_t mode;  // режимы цветовой гаммы: 0-все цвета, 1-белый, 2-желтый
bool LEDW;  // состо€ние белого цвета в цветовой гамме 
bool LEDY; // состо€ние желтого цвета в цветовой гамме

bool DeviceIsON;

uint8_t KEY_MODE_lastState = 0; // запоминание предыдущего состо€ни€ пина в прерывании
uint8_t KEY_ONOFF_lastState = P2;  // запоминание предыдущего состо€ни€ пина в прерывании

uint8_t S = 0; // хранение предыдущего состо€ни€ битов от прерывани€ энкодера



/*const PROGMEM uint8_t br[256]  = {
};*/

// таблица линейной €ркости светодиода
const PROGMEM uint8_t br[147]  = {
};

#endif /* MAIN_H_ */